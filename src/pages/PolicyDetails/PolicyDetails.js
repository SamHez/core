...
function PolicyDetails({ parent_container }) {
  ...
  // Might require to configure service and characteristic UUIDs dynamically from database
  // based on the device each agent or company wants to use/have registered
  const customServiceUuid = '49535343-fe7d-4ae5-8fa9-9fafd205e455';
  const customCharacteristicUuid = '49535343-8841-43f4-a8d4-ecbe34729bb3';

  const connectBluetoothDevice = async () => {
    try {
      const device = await navigator.bluetooth.requestDevice({
        acceptAllDevices: true,
        optionalServices: ['generic_access', 'device_information', customServiceUuid],
      });

      if (!device.gatt) {
        console.error('Error: GATT not available on the connected device.');
        return;
      }

      const server = await device.gatt.connect();
      setDevice({ ...device, server });
      console.log('connectBluetooth printer: started');
      setIsConnected(true);
    } catch (error) {
  ...
  const getPrintFormatText = (policy) => {
    let policyNo = "9000";
    let sticker = policy.stickersDetails[0];
    let clientName = policy.clientDetails.name;
    let company = "SWICO";
    let agentName = policy.added_by_name;
    ...
    bill +=
      "                  " + clientName + "\n" +
      "                  " + policyNo + "\n" +
      "                  " + sticker.plateNo +" "+ sticker.motorMake + "\n" +
      "                  " + sticker.seatingCapacity + "\n" +
      "                  " + sticker.grossWeight + "\n" +
      "                  " + sticker.chasisNo + "\n" +
      "                  " + sticker.totalPremium +" "+ currency + "\n" +
  ...
  const sendTextToPrinter = async () => {
    if (device && device.server) {
      
      const stickerText = getPrintFormatText(policy);
      try {
        const primaryService = await device.server.getPrimaryService(customServiceUuid);
        const characteristic = await primaryService.getCharacteristic(customCharacteristicUuid);
        await characteristic.writeValue(textEncoder.encode(stickerText));
        handleClose();
        submitStickerNo();
      } catch (error) {
    ...
    <div className={isMobile ? "components" : "components detailsMargin"}>
      ...
      <Modal show={show} onHide={handleClose} className="hideOnPrint">
        ...
        {show && (
          <Modal.Body id="stickerPrint">
            <div style={{textAlign: "center", marginBottom: "10px"}}>
              <h2 style={{ fontSize: "0.9rem" }}>Setup Bluetooth Printer</h2>
            </div>
            <Row>
              <Col>
                <Button onClick={connectBluetoothDevice} disabled={isConnected} style={{marginRight: "10px"}}>
                  Connect
                </Button>
                <Button onClick={disconnectBluetoothDevice} disabled={!isConnected} style={{marginLeft: "10px"}}>
                  Disconnect
                </Button>
              </Col>
            </Row>
        ...
        <Modal.Footer className="hideOnPrint">
          ...
          <Button onClick={sendTextToPrinter} disabled={!isConnected}>Print via Bluetooth</Button>
        </Modal.Footer>
      </Modal>
